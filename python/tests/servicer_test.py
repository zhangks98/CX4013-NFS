...
class TestALOServier:
    ...
    def test_handle_register(self, fs: FakeFilesystem):
        ...
        # Register
        self.servicer.handle(req, addr)
        client_identifier = str(addr) + ":" + str(port)
        ...
        assert client_identifier in self.servicer.file_subscriber[combined_path]
        # Should be able to find monitor_interval associated with that client
        assert self.servicer.file_subscriber[combined_path][client_identifier]["monitor_interval"] == monitor_interval
        # Update register
