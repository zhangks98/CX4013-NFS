...
from serialize import INTEGER_BYTES, LONG_BYTES, ByteBuffer
...
class Value:
    ...
    def from_bytes(buffer: ByteBuffer) -> 'Value':
        ...
        elif value_type == ValueType.BYTES.value:
            return Bytes.from_bytes(buffer)
        elif value_type == ValueType.INT32.value:
            return Int32.from_bytes(buffer)
        elif value_type == ValueType.INT64.value:
...
class Int64(Value):
    @classmethod
    def from_bytes(cls, buffer: ByteBuffer):
        return cls(buffer.get_long())
    
    def to_bytes(self):
        return pack('>il', ValueType.INT64.value, self.val)
